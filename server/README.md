...
6. Nella sezione "Environment" del servizio, aggiungi una variabile:
   - Nome: `SECRET_KEY`
   - Valore: una qualsiasi stringa lunga e a caso (es. copiala da un generatore di
     password online) — serve per proteggere gli accessi, non deve essere quella
     di esempio nel codice.
7. Nella sezione "Disks", aggiungi un disco persistente (bastano 1 GB, è nel piano
   gratuito) montato sulla cartella del progetto — **senza questo passaggio i dati
   si cancellano ogni volta che il server si riavvia**, quindi non saltarlo.
